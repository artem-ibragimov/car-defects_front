...
function generate(url) {
    const decoded = decodeURIComponent(url.replaceAll('#entity_params=', '').replaceAll('+', ' '));
    ...
    Promise
        .all(queries.map(([locale, content]) => generateArticle(locale, content, poster, url, cards)))
        .then(() => { info("Articles are done!"); })
        .catch(error);
};
...
readline.question("Enter entity params ", url => {
    readline.close();
    generate(url);
});
...
function generateImg(name) {
    return openai.images.generate({
        ...
        .then(() => { info(`ChatGPT has generated the ${name} image`); })
        .catch(error);
}
