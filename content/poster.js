...
try {
   ...
   post({
      ...
      post: `${json.text.article[poster].text.slice(0, 280 - read_more.length - `[Sent with Free Plan] `.length)}${read_more}`,
      link
   })
      .then(() => { writeFileSync(file, posted.concat(poster).join('\n')); })
      .catch(console.error);
} catch (error) {
   console.error(error);
}
...
function post({ title, post, link, mediaUrls }) {
   return social.post({
      ...
      .then((res) => { if (res.status === 'error') { throw res; } });
}
;
